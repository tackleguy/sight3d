...
Mode: Operate. Preserve the incumbent compact desktop application, system typography, neutral dark surfaces, blue action/selection colors, and model-first viewport. The usability extension does not replace the renderer or drawing interactions.

Sight3D’s document header exposes Save, Undo/Redo, Quick start, Model tray and AI assistant. Essential drawing tools are the default; All tools restores the complete tool inventory. Keyboard shortcuts remain visible in tooltips and desktop rows. A collapsible tool guide connects to AI explanations.
