...
export function validateKnowledgeDesign(input:Record<string,unknown>){
 ...
 return {name,reference,features,parts};
}
export function createKnowledgeDesign(api:IModelAPI,input:Record<string,unknown>){
 const {name,reference,features,parts}=validateKnowledgeDesign(input);
 ...
 return {ok:true,created:{faces},reference,features,parts:parts.length,summary:`Created ${name} as a ${parts.length}-part concept inspired by ${reference}. Design intent: ${features.join('; ')}. This uses the model's general knowledge, not a retrieved image or verified blueprint. Inspect the approximation; Undo reverses the entire design.`};
}
