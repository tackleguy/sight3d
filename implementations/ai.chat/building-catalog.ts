...
const groups:Array<[string,string,Row[]]> = [
  ...
  ['Sport and recreation','civic',[
    ['sports hall',48,32,15,1,'gable','canopy'],['indoor arena',80,60,28,3,'dome','canopy'],['swimming pool building',56,32,12,1,'flat','skylights','natatorium'],['fitness center',28,22,10,2,'flat','shopfront','gym'],['clubhouse',24,18,8,2,'hip','balconies'],
  ]],
...
export function buildingRecipe(archetypeId:string,designStyle:DesignStyle='contemporary',form:MassingForm='compact'):BuildingRecipe {
  ...
  return {id:`${a.id}/${designStyle}/${form}`,name:`${style.label} ${a.name} · ${massing.label}`,archetype:a,designStyle,form,defaults:{type:a.baseType,width:a.width*massing.width,depth:a.depth*massing.depth,height:a.height*massing.height,floors:a.floors,shape,roof:style.roof??a.roof,style:a.id==='greenhouse'?'glass':style.style,taper:massing.taper,
...
export function resolveBuildingRecipe(input:Record<string,unknown>):BuildingRecipe|undefined {
  ...
  const mentioned=findBuildingArchetype(brief);
...
export function searchBuildingCatalog(input:Record<string,unknown>={}) {
  ...
  return {recipeCount:BUILDING_RECIPE_COUNT,subtypeCount:BUILDING_ARCHETYPES.length,matchCount:ranked.length,offset,nextOffset:offset+limit<ranked.length?offset+limit:null,
    ...
    note:'100 concept subtypes × 10 style treatments × 10 massing forms. Recipes are editable exterior concepts, not 10,000 independent building classes, trained weights, construction plans or exact historic reconstructions.'};
}
export function buildingCatalogContext(text:string):string {
  const recipe=resolveBuildingRecipe({brief:text});
  if(!recipe)return /catalog|building types|types of buildings/i.test(text)?`\nLocal building catalog: ${BUILDING_RECIPE_COUNT} exterior concept recipes, from 100 subtypes, 10 styles and 10 forms. Categories: ${groups.map(([name])=>name).join(', ')}. Use search_building_catalog to browse or search; it does not modify geometry.`:'';
