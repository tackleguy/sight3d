import { browserRequest, parseBrowserReply, completedBrowserBoxes } from '../../../src/web/browser-ai-protocol';
...
test('successful box receipts end the plan without repeating geometry',()=>{
 ...
 const result=completedBrowserBoxes({system:'',tools,messages});
 expect(result?.stop_reason).toBe('end_turn');
 expect(result?.content?.[0].text).toContain('2 × 3 × 4 m');
 receipt.content=JSON.stringify({ok:false,error:'Invalid dimensions'});
 expect(completedBrowserBoxes({system:'',tools,messages})?.error).toContain('Invalid dimensions');
});
test('unmatched results never claim a completed box',()=>{
 expect(completedBrowserBoxes({system:'',tools,messages:[{role:'user',content:'Create a box'}]})).toBeNull();
});
